...
#+begin_src emacs-lisp
  (use-package wakib-keys
    ...
    (defun wakib-custom-vterm-aware-backward-delete-word (arg)
      "Will do a regular backward delete if inside vterm.
      Otherwise, it will perform `wakib-custom-backward-delete-word'."
      (interactive "p")
      (if (and (fboundp 'vterm-send-key) (derived-mode-p 'vterm-mode))
          (vterm-send-key (kbd "C-h"))
        (wakib-custom-backward-delete-word arg)))
    ...
    (setq wakib-keylist
          `(("M-j" . left-char)
            ...
            ("<C-backspace>" . wakib-custom-vterm-aware-backward-delete-word)
