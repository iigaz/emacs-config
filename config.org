...
obvious, and sometimes messes up customizations done later, for
example here. This code will move the default storage to another file
and will load it, before any calls of the use-package, so that
selected packages would be determined by this configuration, not
random packages I installed once and forgot about.
...
#+begin_src emacs-lisp
  ...
  ;; Empty package-selected-packages
...
ones. I don't like cua-mode either. The best alternative I've found is
...
from vscode, with some functions hacked together by me from various
snippets from internet.
...
#+begin_src emacs-lisp
  ...
  ;; Flip horizontal scroll direction
...
replicate the ones I'm used to, coming from VSCode.
...
#+begin_src emacs-lisp
  ...
  ;; Do not spam dired buffers when opening folders
...
useful when you're writing lisp.
...
A cool mode for writing rest query templates.
