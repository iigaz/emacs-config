...
#+begin_src emacs-lisp
  ...
  and ignore-project is nil, `async-shell-command' otherwise."
    ...
    (wakib-define-keys overriding-terminal-local-map '(("C-c" . wakib-custom-copy-without-region)
                                                       ("C-x" . wakib-custom-cut-without-region)))
    ...
    ;; REDEFINING ENTIRE `goto-address-fontify' FUNCTION JUST TO CHANGE HELP MESSAGES
          ...
          (while (re-search-forward goto-address-url-regexp end t)
            (let* ((s (match-beginning 0))
              ...
              (when (or (not goto-address-prog-mode)
                ...
                (overlay-put this-overlay
                             'help-echo "mouse-3, C-mouse-1, C-d C-o: follow URL")
                (overlay-put this-overlay
          ...
          (while (re-search-forward goto-address-mail-regexp end t)
            (let* ((s (match-beginning 0))
              ...
              (when (or (not goto-address-prog-mode)
                ...
                (overlay-put this-overlay 'follow-link t)
                (overlay-put this-overlay
                             'help-echo "mouse-3, C-mouse-1, C-d C-o: mail this address")
                (overlay-put this-overlay
                             'keymap goto-address-highlight-keymap)
                (overlay-put this-overlay 'goto-address t))))))))
#+end_src

** Window management :modulated:
...
#+begin_src emacs-lisp
  (use-package dired
    ...
    :bind (:map dired-mode-map ("<mouse-2>" . dired-mouse-find-file)))
...
** Session management :wont_be_modulated:
...
** Helpful niceties :modulated:

[[https://github.com/justbur/emacs-which-key][Which-key]] is a helpful window that pops up when you start pressing
some key chord.
[[https://github.com/minad/consult][Consult]] adds various search functions.
[[https://github.com/radian-software/ctrlf][CTRLF]] is a package that replaces interactive search.
...
[[https://github.com/elizagamedev/shell-command-x.el][shell-command-x]] improves shell-command and async-shell-command
experience by renaming buffers and allowing to quickly restart
commands.
[[https://github.com/yoshiki/yaml-mode][yaml-mode]] is just a major mode for YAML files, I don't know why it
isn't in the default Emacs.
...
** Minibuffer and completion :modulated:

Here I'm using [[https://github.com/minad][Minad]] stack for minibuffer completion. Although there's
a lot of packages and some have alternatives built in Emacs, they load
fast and have some nice features so it's worth installing them all.
...
** Select light / dark theme based on system theme :modulated:
...
** Completion in buffer :modulated:
...
[[https://github.com/minad/corfu][Corfu]] is a modern and lightweight frontend for CAPFs. It can be used
as an alternative to company, but unfortunately I could not get it to
work flawlessly in Emacs 29.4 built with pgtk (it would frequently
freeze and sometimes crash Emacs). So for now, I've decided to settle
on them both, company for auto-completion and corfu for manual
completion.
