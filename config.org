...
- It may be better to use built-in ~fido-vertical-mode~ instead of
  ~vertico~. It still works with marginalia (quite useful and I don't
  know whether it has a built-in replacement) and orderless (also
  cool).
- I don't want to install consult only to use it for buffer
  switching. May be worth to explore what built-in options can offer.
